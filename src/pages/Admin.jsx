...
const QUILL_MODULES = {
  toolbar: {
    ...
    handlers: {
      image: function () {
        const url = prompt("Vložte URL obrázku (např. z Supabase Storage):");
        if (url) {
...
export default function Admin() {
  ...
  const load = async () => {
    ...
    if (error) {
      setDbError(`Chyba načítání: ${error.message}`);
    } else {
      setProducts(data || []);
    }
    setLoading(false);
  ...
  // --- SCRAPER → Vercel API route /api/scrape ---
    ...
    try {
      ...
      if (data.html) {
        setForm(f => ({
          ...
          name: f.name || data.product_name || f.name,
        }));
      }
    } catch (err) {
  ...
  // --- DOCX IMPORT → Vercel API route /api/docx-to-html ---
  const importDocx = async (file) => {
    ...
    try {
      ...
      if (data.html) {
        setForm(f => ({ ...f, description: data.html }));
      }
    } catch (err) {
  ...
  const save = async () => {
    ...
    const payload = {
      ...
      inquiry_only: !!form.inquiry_only,
      active: !!form.active,
  ...
  const handleLogin = async (e) => {
    ...
    setLoginError("");
    try {
      await login(emailInput, pwInput);
    } catch {
        ...
        <form onSubmit={handleLogin} className="space-y-3">
          <div>
            ...
            <input type="email" value={emailInput} onChange={e => setEmailInput(e.target.value)} required
              placeholder="admin@vahy-dychl.cz"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={openNew}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-blue-600 text-white font-bold hover:opacity-80 transition-all">
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-semibold text-slate-600">Kategorie</th>
                ...
                {filtered.map(p => (
                  <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      {p.certified ? (
                        <span className="inline-flex items-center justify-center w-5 h-5 rounded text-xs font-black text-white mx-auto" style={{ background: "#16a34a" }}>M</span>
                      ) : <span className="text-slate-300">—</span>}
                    </td>
                    <td className="px-4 py-3 text-center">
                      {p.active ? <Check className="w-4 h-4 text-green-500 mx-auto" /> : <X className="w-4 h-4 text-red-400 mx-auto" />}
                    </td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center gap-2 justify-end">
                        ...
                        <button onClick={() => remove(p.id)} className="p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-2xl bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={scrapeProduct} disabled={scrapeLoading || !scrapeUrl.trim()}
                    className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-blue-600 text-white text-sm font-bold hover:opacity-80 disabled:opacity-50 transition-all">
                ...
                <p className="text-xs text-slate-400 mt-1">AI automaticky extrahuje celý popis a specifikace z URL produktu.</p>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <label className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-semibold cursor-pointer transition-all ${docxLoading ? "border-blue-300 text-blue-500 bg-blue-50" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}>
                    ...
                    <input ref={docxRef} type="file" accept=".docx" className="hidden" disabled={docxLoading}
                ...
                <p className="text-xs text-slate-400 mt-1">Podporuje formátování. Pro import z Word dokumentu použijte tlačítko výše.</p>
              </div>
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
              <button onClick={closeForm} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 font-semibold hover:bg-slate-50 transition-all text-sm">
                Zrušit
              </button>
